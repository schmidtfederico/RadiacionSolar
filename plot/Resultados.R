...
plotear.resultados <- function(resultados, nombre.estacion) {
    ...
    # Bristow-Campbell
    plotear.con.ajuste(x=resultados$data, y=resultados$bc, ylab="Radiación Estimada Por Bristow-Campbell",
                       titulo=titulo.plot, bg="lavender")
    
    # Hargreaves
    plotear.con.ajuste(x=resultados$data, y=resultados$ha, ylab="Radiación Estimada Por Hargreaves",
                       titulo=titulo.plot, bg="darkseagreen1")
    
    # Mahmoood-Hubbard
    plotear.con.ajuste(x=resultados$data, y=resultados$mh, ylab="Radiación Estimada Por Mahmood-Hubbard",
    ...
    plotear.con.ajuste(x=resultados$data, y=resultados$mh, ylab="Radiación Estimada Por Angstrom-Prescott",
                       titulo=titulo.plot, bg="bisque")
}


plotear.con.ajuste <- function(x, y, ylab, bg, titulo) {
    plot(x=x, y=y, bg=bg, pch=21, cex=0.5, lwd=0.5, main=titulo, xlab="Radiación Medida", ylab=ylab)
    abline(0,1, lwd=1.5)
    ...
    # Residuales
    plot(x=x, y=fit$residuals, pch=21, cex=0.5, lwd=0.5, main=titulo, xlab=ylab, ylab="Residuales", bg=bg)
    abline(0,0, lwd=1.5)
